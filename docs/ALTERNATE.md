...
> [!NOTE]
> **Superseded.** This first attempt at a second derivation stops where candidate points on the two
> circles must be matched. Its constraints remain correct and provide the starting point for the
> current method. Three constraints are linear in $O_3$, reducing the matching problem to one
> scalar equation in one unknown. See [LINEAR_METHOD.md](LINEAR_METHOD.md). This file is retained
