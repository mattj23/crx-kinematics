...
# ROBOT_JOINTS = [10, -80, 10, 60, -90, 45]
...
def intersections(robot: Robot, angle: float):
    ...
    if d > robot.z1:
        raise ValueError("No solutions for O2")

    r = numpy.sqrt(robot.z1 ** 2 - d ** 2)
    # o1_circle = f_1.transform_points(_xy_circle(r, 1000, x=o1_proj.x, y=o1_proj.y))
    o3_circle = f_1.transform_points(xy_circle(robot.x1, 500))

    o3_2d = Circle2(0, 0, robot.x1)
    o1_2d = Circle2(o1_proj.x, o1_proj.y, r)

    inter = [f_1 @ Point3(*p, 0.0) for p in o3_2d.intersections_with(o1_2d)][:1]

    return o4, inter, o3_circle
...
def main():
    ...
    # o4, o3_circle, o1_circle, inter = intersections(robot, numpy.pi / 0.7)

    # helper.add_points(o4, color="blue", point_size=10, render_points_as_spheres=True)
    # plotter.add_lines(o3_circle, connected=True, color="blue", width=3.0)
    # plotter.add_lines(o1_circle, connected=True, color="green", width=3.0)
    #
    # helper.add_points(*inter, color="orange", point_size=10, render_points_as_spheres=True)
    STEPS = 500
    step_points = []
    leg_traces = []
    for i in range(STEPS):
        angle = (i / STEPS) * 2.0 * numpy.pi
        try:
            o4, inter, o3_circle = intersections(robot, angle)
            # plotter.add_lines(o3_circle, connected=True, color="orange", width=1.0)
            for p in inter:
                leg_traces.append(list(o4.coords))
                leg_traces.append(list(p.coords))

            step_points.extend(inter)
        except ValueError:
            pass
    leg_traces = numpy.array(leg_traces)
    plotter.add_lines(leg_traces, color="magenta", width=1.0)
    helper.add_points(*step_points, color="orange", point_size=10, render_points_as_spheres=True)
